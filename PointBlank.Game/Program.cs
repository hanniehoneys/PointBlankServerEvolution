            ...
            try
            {
              ...
              else if (account.access != AccessLevel.Banned)
              {
                if (ComDiv.updateDB("accounts", "access_level", (object) -1, "player_id", (object) account.player_id))
                {
                  ...
                  using (PROTOCOL_LOBBY_CHATTING_ACK lobbyChattingAck = new PROTOCOL_LOBBY_CHATTING_ACK("Server", 0U, 0, true, "แบนผู้เล่น [" + account.player_name + "] ถาวร - ใช้โปรแกรมช่วยเล่น"))
